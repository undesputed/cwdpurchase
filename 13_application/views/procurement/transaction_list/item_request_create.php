...
<div class="t-content">
	<div class="t-header">
		<a href="<?php echo base_url().index_page(); ?>/transaction_list/item_transfer/item_request" class="close"><span aria-hidden="true">&times;</span><span></a>
	...
	var item_issuance_app = {
		init:function(){
			...
			$('#request_to').on('change',function(){
				...
				$post = {
					location : $('#request_to option:selected').val(),
				};
			...
			$.post('<?php echo base_url().index_page();?>/procurement/item_transfer/save_request',$post,function(response){
			...
			if($('#item_list tbody tr td').find('#item_select').html()==''){
				return true;
			}else{
				return false;
			}
