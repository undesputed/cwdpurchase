...
<?php if(count($pr_list)>0): ?>
	  		<?php foreach($pr_list as $key=>$row): ?>
				 ...
				 <?php 	
					$active = ($row1['purchaseNo'] == $_active ) ? 'act': '';
				 ?>
							<div class="item_contents pop_up_content hover <?php echo $active; ?>" data-method="purchase_request" data-type="incoming" data-value="<?php echo $row1['purchaseNo']; ?>" data-url="<?php echo base_url().index_page(); ?>/transaction_list/purchase_request/incoming/<?php echo $row1['purchaseNo']; ?>">
	  ...
	  <?php if($next!="" || $next > 1): ?>
		<a class="jscroll-next" href='<?php echo base_url().index_page(); ?>/transaction_list/get_pr_incoming?p=<?php echo $next; ?>&filter=<?php echo $filter.$search_url; ?>'>Next</a>
	  <?php endif; ?>
