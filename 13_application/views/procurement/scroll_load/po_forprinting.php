			...
			<?php if(count($po_list) > 0): ?>
		  		<?php foreach($po_list as $key=>$row): ?>
					...
					<div class="head_date">
					 ...
					 <?php foreach($row as $row1): ?>
							...
							<div class="item_contents hover pop_up_content" data-method="purchase_order" data-type="for_printing" data-value="<?php echo $row1['po_number']; ?>" data-url="<?php echo base_url().index_page(); ?>/transaction_list/purchase_order/for_printing/<?php echo $row1['po_number']; ?>" >
	  		...
	  		<?php if($next!=""): ?>
			  <a class="jscroll-next" href='<?php echo base_url().index_page(); ?>/transaction_list/get_po_forprinting?p=<?php echo $next; ?>&filter=<?php echo $filter.$search_url; ?>'>Next</a>
			<?php endif; ?>