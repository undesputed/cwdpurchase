	...
	<div class="row">
			<div class="col-md-3">
				...
				<div class="panel panel-default">		
					  <div class="panel-body">
					  		<div class="form-group">
					  			<button id="new_request" class="btn btn-primary btn-block">Transfer an Equipment</button>
					  		</div>
					  </div>	 
				</div>
...
<script type="text/javascript">
	...
	var app = {
		init:function(){
			...
			$.post('<?php echo base_url().index_page(); ?>/fixed_asset/equipment_transfer/get_cumulative',$post,function(response){
			...
			$post = {
				...
				id           : $(this).find('td.equipment_request_id').text(),
				
			};

			$.post('<?php echo base_url().index_page();?>/fixed_asset/equipment_transfer/get_details',$post,function(response){
				$.fancybox(response,{
					width     : 1200,
					height    : 550,
					fitToView : false,
			...
			$.fancybox.showLoading();
			$.post('<?php echo base_url().index_page();?>/fixed_asset/equipment_transfer/new_request',$post,function(response){
				$.fancybox(response,{
					width     : 1200,
					height    : 550,
					fitToView : false,
