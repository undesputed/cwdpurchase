	...
	<div class="row">
			<div class="col-md-3">
				...
				<div class="panel panel-default">		
					  <div class="panel-body">
					  		<div class="form-group">
					  			<button id="new_request" class="btn btn-primary btn-block">Register an Equipment</button>
					  		</div>
					  </div>	 
				</div>
...
<script type="text/javascript">
	...
	var app = {
		init:function(){
			...
			$.post('<?php echo base_url().index_page(); ?>/fixed_asset/equipment_registry/get_cumulative',$post,function(response){
			...
			$post = {
				...
				id           : $(this).find('td.Equip_ID').text(),
				
			};

			$.post('<?php echo base_url().index_page();?>/fixed_asset/equipment_registry/get_details',$post,function(response){
				$.fancybox(response,{
					width     : 1200,
					height    : 550,
					fitToView : false,
			...
			$.fancybox.showLoading();
			$.post('<?php echo base_url().index_page();?>/fixed_asset/equipment_registry/new_request',$post,function(response){
				$.fancybox(response,{
					width     : 1200,
					height    : 550,
					fitToView : false,
