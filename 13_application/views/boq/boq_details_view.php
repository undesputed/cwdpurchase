							...
							<div class="row">
								<div class="col-md-6">
									<div class="form-group">
										...
										<input type="hidden" id="txtproject_id" name="txtproject_id" value="<?php echo $boq_main_new['project_id']; ?>" />
									</div>
								</div>
								<div class="col-md-6">
									<div class="form-group">
...
<script type="text/javascript">
	$(function(){
		...
		function change_item_slct() { 
			$('body').on('change', '#item', function() { 
				var data = $(this).select2('data');
				$('#unit').val(data.unit);
				$('#item_description').val(data.text);
			});
		}
		...
		function ajax_show_boq_details() { 
			$.get(base_url+'boq/ajax_show_boq_details/<?php echo $boq_main_new['id']; ?>/view_only', function(res) { 
		...
		function save_purchase_request() { 
			$('body').on('click', '#btn-save-pr', function() { 
				var project_id = $('#txtproject_id').val();
				$.post(base_url+'boq/ajax_process_save_new_pr/'+project_id, $('#frm-purchase-request').serialize(), function(res) { 
					var obj_res = $.parseJSON(res);
					if ( obj_res.ctr ) {
