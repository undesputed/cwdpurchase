	  ...
	  <script>
		...
		var cash_voucher = function(){
			$.fancybox.showLoading();
			$.get('<?php echo base_url().index_page() ?>/accounting/cash_voucher',function(response){
				$.fancybox(response,{
					width     : 1300,
					height    : 650,
					fitToView : false,
		...
		var journal_voucher = function(){
			$.fancybox.showLoading();
			$.get('<?php echo base_url().index_page() ?>/accounting/journal_voucher',function(response){
				$.fancybox(response,{
					width     : 1300,
					height    : 650,
					fitToView : false,
				...
				<div class="dropdown search-item" style="float:left;position:relative;z-index:9999">
					<a href="javascript:void(0)" id="filter"  class="search-item dropdown-toggle" data-toggle="dropdown" aria-expanded="false"><i class="fa fa-filter"></i> Filter </a>
					    <ul class="dropdown-menu filter-menu" role="menu">
					      ...
					      <li><a href="javascript:void(0)" class="filter-item "  data-url  = "<?php echo base_url().index_page(); ?>/transaction_list/get_voucher_list?p=1?&filter=journal" >Journal  <i class="fa fa-check"></i></a></li>		
					    </ul>
				</div>
			</div>
			<div class="col-md-6">
				...
				<div class="btn-group pull-right" style="margin:5px">
				  <button type="button" class="btn btn-sm btn-primary dropdown-toggle" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
				    Create Voucher <span class="caret"></span>
				  </button>
				  <ul class="dropdown-menu">
				    <li><a href="javascript:void(0)" onclick="cash_voucher()">Disbursement Voucher</a></li>
				    <li><a href="javascript:void(0)" onclick="journal_voucher()">Journal Voucher</a></li>
				  </ul>
				</div>
