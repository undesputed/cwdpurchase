...
class Boq extends CI_Controller {
	...
	public function boq_view() { 
		$this->lib_auth->title = "BOQ Projects";
		$this->lib_auth->build = "boq/boq_view";
	...
	public function boq_details_view($boq_main_id) { 
		$this->lib_auth->title = "BOQ Project Details";
		$this->lib_auth->build = "boq/boq_details_view";
	...
	public function ajax_show_boq_view() { 
		if ( !$this->input->is_ajax_request() ) { exit('Invalid access...'); }
		
		$project_id = trim($this->input->post('project'));
		$project_type_id = trim($this->input->post('project_type'));
		...
		}elseif($this->session->userdata('type_user') == 'USER'){
			$where = array( 
				'status' => 'COMPLETED', 
		...
		$boq_records = $this->md_boq->get_boq_records($where); ?>
						...
						<tr>
							...
							<td>
								<?php if ( $this->md_boq->has_existing_boq_details($boq->id) ) { ?>
									<a class="view-boq-details pointer" title="View BOQ Details" href="index.php/boq/boq_details_view/<?php echo $boq->id; ?>"><i class="glyphicon glyphicon-th-list"></i></a>
								<?php } ?>
							</td>
						</tr>
				<?php 
...
<?php 
	...
	public function ajax_show_boq_details( $boq_main_id, $type='view_all' ) { 
		if ( !$this->input->is_ajax_request() ) { exit('Invalid access!...'); }
				...
				<table class="table table-boq-details">
					<thead>
						<tr>
							...
							<td class="thead" rowspan="2">&nbsp;</td>
							<td class="thead" rowspan="2">&nbsp;</td>
							<td class="thead" colspan="<?php echo ( $type == 'view_all' ) ? '5' : '2'; ?>">UNIT COST</td>
